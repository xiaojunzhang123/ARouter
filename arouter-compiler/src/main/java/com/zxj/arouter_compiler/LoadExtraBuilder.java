...
public class LoadExtraBuilder {
    ...
    public LoadExtraBuilder(ParameterSpec parameterSpec) {
        //定义函数方法模型  public void loadExtra(Object target)
    ...
    private void addArrayStatement(String statement, String fieldName, String extraName,
        ...
        switch (typeMirror.toString()){
            ...
            default:
                //Parcelable 数组
                String defaultValue = "t." + fieldName;
                //object 数组 componentType 获得 object 类型
                ArrayTypeName arrayTypeName = (ArrayTypeName) ClassName.get(typeMirror);
                TypeElement typeElement = elementUtils.getTypeElement(
                    arrayTypeName.componentType.toString());
                //是否为 Parcelable 类型
    ...
    private void addObjectStatement(String statement, String fieldName, String extraName,
        ...
        if (typeUtils.isSubtype(typeMirror, parcelableType)) {
        ...
        } else if (typeMirror.toString().endsWith(Constant.STRING)) {
            ...
            builder.addStatement(statement, TypeName.get(element.asType()), Constant.ROUTER,
                extraName);
            return;
        } else {
            //List
            ...
            if (typeName instanceof ParameterizedTypeName) {
                ...
                if (!rawType.toString().equals(Constant.ARRAYLIST) && !rawType.toString().equals(
                    Constant.LIST)) {
                    throw new RuntimeException(
                        "not support inject type " + typeMirror + " " + element);
                }
                ...
                if (typeUtils.isSubtype(typeElement.asType(), parcelableType)) {
                    ...
                    throw new RuntimeException(
                        "Not Support Generic Type " + typeMirror + " " + element);
                }
            } else {
